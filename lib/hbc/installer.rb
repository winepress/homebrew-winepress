...
class Hbc::Installer
  ...
  def prefix_configure
    odebug "Configuring Prefix"

    ENV['WINEPREFIX'] = @cask.staged_path.join("prefix")
    ENV['WINEDEBUG'] = '-all'

    if ! @cask.wine_arch.nil?
      ENV['WINEARCH'] = @cask.wine_arch
    end
  ...
  def verify_wine
    wine_version = @cask.wine_version

    installed = @command.run(Hbc.homebrew_executable,
      :args => ['list', '--versions', 'wine'],
      :print_stderr => false).stdout.include?('wine')

    updated = @command.run(Hbc.homebrew_executable,
      :args => ['list', '--versions', 'wine'],
      :print_stderr => false).stdout.include?('wine #{wine_version}')

    if not installed
      puts "Installing Wine"
      install_wine
    # elsif not updated
      # puts "Updating wine to #{wine_version}"
      # upgrade_wine
    end
  end
