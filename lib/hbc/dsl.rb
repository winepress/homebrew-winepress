...
module Hbc::DSL
  ...
  def wine_version; self.class.wine_version; end

  def wine_arch; self.class.wine_arch; end

  def wine_tricks; self.class.wine_tricks; end
  ...
  module ClassMethods
    ...
    def wine_version(wine_version=nil)
      if @wine_version and !wine_version.nil?
        raise Hbc::CaskInvalidError.new(self.token, "'wine_version' stanza may only appear once")
      end
      @wine_version ||= wine_version
    end

    def wine_arch(wine_arch=nil)
      if @wine_arch and !wine_arch.nil?
        raise Hbc::CaskInvalidError.new(self.token, "'wine_arch' stanza may only appear once")
      end
      @wine_arch ||= wine_arch
    end

    def wine_tricks(wine_tricks=nil)
      if @wine_tricks and !wine_tricks.nil?
        raise Hbc::CaskInvalidError.new(self.token, "'wine_tricks' stanza may only appear once")
      end
      @wine_tricks ||= wine_tricks
    end

    def win_exe(win_exe=nil)
      if @win_exe and !win_exe.nil?
